        ...
        <div className="max-w-4xl mx-auto space-y-6 fade-in pb-20">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-text-secondary">Manage supplier orders.</p>
                </div>
            ...
            <div className="card-luxury overflow-hidden">
                <div className="table-wrapper-dark overflow-x-auto">
                    <table className="table-dark w-full min-w-full">
                        ...
                        <tbody>
                            {loading ? (
                                <tr><td colSpan="5" className="p-8 text-center text-text-secondary animate-pulse">Loading History...</td></tr>
                            ) : history.map(h => (
                                <tr key={h.id}>
                                    <td className="pl-6 font-mono text-xs text-text-secondary">{new Date(h.order_date).toLocaleDateString()}</td>
                                    <td className="text-text-primary font-medium">{h.supplier_name}</td>
                                    ...
                                    <td className="text-right pr-6"><button onClick={() => openDetail(h)} className="text-xs font-bold text-text-secondary hover:text-lumina-gold">Detail</button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-surface/80 backdrop-blur-sm p-4">
                        <div className="card-luxury w-full max-w-4xl p-6 fade-in-up max-h-[90vh] overflow-y-auto flex flex-col">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-lg font-bold text-text-primary">New Purchase Order</h3>
                            ...
                            <form onSubmit={submitPO} className="space-y-4 flex-1 overflow-y-auto">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-text-secondary mb-1">Warehouse</label>
                                ...
                                <div className="bg-background p-4 rounded-xl border border-lumina-border">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                                        <div className="sm:col-span-6">
                                            <label className="text-xs font-bold text-text-secondary mb-1 block">Item</label>
                                        ...
                                        <div className="sm:col-span-3">
                                            <label className="text-xs font-bold text-text-secondary mb-1 block">Cost</label>
                                    ...
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-sm text-left text-text-primary min-w-[400px]">
                                            ...
                                            <tbody className="divide-y divide-lumina-border">
                                                {cart.map((c, idx) => (
                                                    <tr key={idx}>
                                                        <td className="p-3">{c.name} <span className="text-xs text-text-secondary block sm:inline sm:ml-1">{c.sku}</span></td>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-surface/80 backdrop-blur-sm p-4">
                        <div className="card-luxury w-full max-w-2xl p-6 fade-in-up max-h-[90vh] overflow-y-auto">
                            <div className="flex justify-between mb-4">
                                <h3 className="text-lg font-bold text-text-primary">PO Details</h3>
                                <button onClick={()=>setDetailOpen(false)} className="text-text-secondary">✕</button>
                            </div>
                            <div className="space-y-4">
                                <div className="flex flex-col sm:flex-row justify-between text-sm border-b border-lumina-border pb-3 gap-2">
                                    <span className="text-text-secondary">Supplier: <strong className="text-text-primary">{selectedPO.supplier_name}</strong></span>
                                    <span className="text-text-secondary">Status: <strong className="text-text-primary uppercase">{selectedPO.payment_status}</strong></span>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm text-text-primary min-w-[300px]">
                                        ...
                                        <tbody className="divide-y divide-lumina-border">
                                            {poItems.map((i, idx) => (
                                                <tr key={idx}>
                                                    <td className="p-2">{i.name} <span className="text-xs text-text-secondary block">{i.sku}</span></td>
