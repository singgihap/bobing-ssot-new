...
export default function ProductsPage() {
    ...
    return (
        <div className="space-y-6 fade-in pb-20">
            {/* Header Solid & Sticky */}
            <div className="flex flex-col md:flex-row justify-between items-center sticky top-0 z-20 bg-surface py-4 px-4 md:px-8 border-b border-lumina-border/50 shadow-md md:static">
            <div className="flex flex-col w-full md:w-auto">
                <h2 className="text-xl md:text-3xl font-bold text-text-primary flex-shrink-0">
                Product Models
                </h2>
                <p className="hidden md:block text-sm text-text-secondary mt-2 font-light">
            ...
            <div className="flex w-full md:w-80 flex-row items-center gap-2 mt-2 md:mt-0">
                ...
                <input
                    ...
                    className="w-full pl-10 bg-transparent text-text-primary text-sm py-2 outline-none placeholder:text-text-secondary/50 font-mono rounded-full border border-lumina-border"
                />
                </div>
                <button
                onClick={() => openModal()}
            ...
            <div className="hidden md:block card-luxury overflow-hidden">
                <table className="table-dark w-full">
                    ...
                    <tbody>
                        {loading ? <tr><td colSpan="7" className="p-12 text-center text-text-secondary animate-pulse">Loading...</td></tr> : filtered.map(p => {
                                ...
                                <React.Fragment key={p.id}>
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-xs text-text-secondary mt-1">{p.category}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-right pr-8" onClick={(e) => e.stopPropagation()}>
                                            <div className="flex justify-end gap-2">
                                                <button onClick={() => openModal(p)} className="text-[10px] uppercase font-bold text-text-secondary hover:text-text-primary border border-lumina-border hover:border-white rounded px-2 py-1 transition-colors">EDIT</button>
                                                    ...
                                                    <div className="border border-lumina-border rounded-lg overflow-hidden w-full bg-surface">
                                                        <table className="w-full text-sm text-left">
                                                            ...
                                                            <tbody className="divide-y divide-lumina-border">
                                                                {loadingVariants ? (
                                                                    <tr><td colSpan="5" className="p-4 text-center text-xs text-text-secondary animate-pulse">Loading variants...</td></tr>
                                                                    ...
                                                                    <tr key={v.id} className="hover:bg-lumina-highlight/20 transition-colors">
                                                                        ...
                                                                        <td className="px-4 py-3 text-text-primary">{v.color}</td>
                                                                        <td className="px-4 py-3 text-text-primary">{v.size}</td>
                                                                        <td className="px-4 py-3 text-right font-mono font-medium text-text-primary">{formatRupiah(v.price)}</td>
                                                                        <td className="px-4 py-3 text-right font-mono text-text-secondary">{formatRupiah(v.cost)}</td>
                                                                    </tr>
                                                                ))}
                                                                {(!loadingVariants && (!variantsCache[p.id] || variantsCache[p.id].length === 0)) && (
                                                                     <tr><td colSpan="5" className="p-4 text-center text-xs text-text-secondary">No variants found.</td></tr>
                                                                )}
                                                            </tbody>
                                                        </table>
                                                    </div>
                    ...
                    <div key={p.id} onClick={()=>toggleVariants(p.id)} className="card-luxury p-4 active:scale-[0.98] transition-transform">
                         <div className="flex gap-4 items-center">
                            <div className="w-16 h-16 rounded-lg bg-surface border border-lumina-border flex-shrink-0 overflow-hidden">
                                {p.image_url ? <img src={p.image_url} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-xs text-text-secondary">IMG</div>}
                            </div>
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-text-secondary mt-0.5">{p.brand_name} • {p.category}</p>
                            </div>
                         </div>
                         ...
                         {expandedProductId===p.id && (
                             <div className="mt-3 bg-surface rounded-lg border border-lumina-border p-2 animate-fade-in">
                                 <div className="text-[10px] text-text-secondary mb-2 uppercase tracking-wider font-bold">Varian Tersedia</div>
                                     ...
                                     {(variantsCache[p.id]||[]).sort(sortBySize).map(v=>(
                                         <div key={v.id} className="flex justify-between items-center text-xs border-b border-lumina-border/50 pb-1 last:border-0 last:pb-0">
                                             <div>
                                                 ...
                                                 <div className="text-text-secondary">{v.color} / {v.size}</div>
                                             </div>
                                             <div className="font-bold">{formatRupiah(v.price)}</div>
                                         </div>
                                     ))}
                                     {loadingVariants && <div className="text-center text-xs py-2 text-text-secondary">Memuat varian...</div>}
                                 </div>
                             </div>
                         )}
                    </div>
            ...
            <Portal>
            {modalOpen && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-surface/80 backdrop-blur-sm p-4">
                    <div className="bg-surface border border-lumina-border rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
                        <div className="px-6 py-4 border-b border-lumina-border flex justify-between items-center">
                            <h3 className="text-lg font-bold text-text-primary">{formData.id ? "Edit" : "Baru"}</h3>
                            <button onClick={() => setModalOpen(false)} className="text-2xl text-text-secondary">×</button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                            {/* Existing Form Content */}
                            <div className="flex items-center gap-4 p-4 bg-surface rounded-lg border border-dashed border-lumina-border">
                                <div className="w-20 h-20 rounded-lg bg-surface flex items-center justify-center overflow-hidden border border-lumina-border relative">
