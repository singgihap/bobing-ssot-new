        ...
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-7rem)] fade-in relative pb-20 lg:pb-0">
            ...
            <div className={`w-full lg:w-2/3 flex flex-col gap-4 h-full ${activeMobileTab === 'products' ? 'flex' : 'hidden lg:flex'}`}>
                ...
                <PageHeader title="Point of Sales" subtitle="Kasir & Transaksi Cepat">
                    <div className="bg-[#12141C] p-2 rounded-xl border border-lumina-border/50 flex flex-col md:flex-row gap-2 items-center w-full md:w-auto">
                        <div className="relative flex-1 w-full">
                            <input 
                                ref={searchInputRef} 
                                type="text" 
                                className="input-luxury pl-10 py-2 text-sm bg-[#0B0C10]" 
                                placeholder="Cari (F2)..." 
                                value={searchTerm} 
                        ...
                        <select className="input-luxury w-full md:w-32 bg-[#0B0C10] py-2 text-xs" value={selectedWh} onChange={e=>setSelectedWh(e.target.value)}>
                ...
                <div className="flex-1 overflow-y-auto pr-1 scrollbar-hide grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 content-start pb-24 lg:pb-0">
                    {loading ? <div className="col-span-full text-center py-10 text-lumina-muted">Loading...</div> : filteredProducts.map(p => {
                        ...
                        return (
                            <div key={p.id} onClick={() => { setSelectedProdForVariant(p); setModalVariantOpen(true); }} className={`card-luxury p-3 md:p-4 cursor-pointer hover:border-lumina-gold/50 transition-all flex flex-col justify-between group active:scale-95 bg-[#12141C] border-lumina-border/30 ${stock<=0?'opacity-50':''}`}>
                                <div>
                                    ...
                                    <h4 className="text-xs md:text-sm font-bold text-white group-hover:text-lumina-gold line-clamp-2 leading-tight min-h-[2.5em]">{p.name}</h4>
                                </div>
            ...
            <div className={`w-full lg:w-1/3 card-luxury flex flex-col h-full overflow-hidden border-lumina-border/50 bg-[#12141C] ${activeMobileTab === 'cart' ? 'flex fixed inset-0 z-30 lg:static' : 'hidden lg:flex'}`}>
                
                <div className="p-5 border-b border-lumina-border/50 bg-[#12141C] flex justify-between items-center shrink-0 shadow-md z-10">
                    ...
                    <button onClick={()=>setCart([])} className="text-xs text-rose-400 hover:text-white border border-rose-500/30 hover:bg-rose-500/20 px-3 py-1.5 rounded-lg transition-all">RESET (F8)</button>
                </div>

                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#0B0C10]/50 scrollbar-hide">
                    {cart.length === 0 ? (
                        ...
                        <div key={idx} className="flex justify-between items-center bg-[#181A25] p-3 rounded-xl border border-white/5 hover:border-lumina-gold/30 transition-all shadow-sm animate-fade-in">
                            <div className="flex-1 mr-3">
                                ...
                                <div className="text-xs text-lumina-muted mt-1 flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] bg-white/10 px-1.5 rounded text-white/70">{item.spec}</span>
                                </div>
                                <div className="text-sm font-bold text-emerald-400 mt-1">{formatRupiah(item.price)}</div>
                            </div>
                            <div className="flex items-center bg-[#0B0C10] rounded-lg border border-white/10 shadow-inner">
                                <button onClick={() => { const n = [...cart]; if(n[idx].qty > 1) n[idx].qty--; else n.splice(idx, 1); setCart(n); }} className="w-8 h-8 flex items-center justify-center text-lumina-muted hover:text-white hover:bg-white/5 rounded-l-lg transition-colors">-</button>
                                <span className="text-sm font-bold w-8 text-center text-white">{item.qty}</span>
                                <button onClick={() => { if(item.qty < item.max) { const n = [...cart]; n[idx].qty++; setCart(n); } else { toast.error('Stok max'); } }} className="w-8 h-8 flex items-center justify-center text-lumina-muted hover:text-white hover:bg-white/5 rounded-r-lg transition-colors">+</button>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {/* BILLING AREA */}
                <div className="p-5 border-t border-lumina-border bg-[#12141C] space-y-4 shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] z-20">
                    <div className="grid grid-cols-2 gap-3">
                        <select className="input-luxury py-2 text-xs bg-[#0B0C10]" value={selectedCustId} onChange={e=>setSelectedCustId(e.target.value)}>
                            <option value="">👤 Tamu (Guest)</option>
                            {customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}
                        </select>
                        <select className="input-luxury py-2 text-xs bg-[#0B0C10]" value={paymentAccId} onChange={e=>setPaymentAccId(e.target.value)}>
                    ...
                    <div className="space-y-1 py-2">
                        <div className="flex justify-between items-end">
                            ...
                            <span className="text-3xl font-display font-bold text-white">{formatRupiah(cartTotal)}</span>
                        </div>
                    </div>

                    <div className="bg-[#0B0C10] p-3 rounded-xl border border-lumina-border/50 flex flex-col gap-2">
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                        <div className="card-luxury w-full max-w-lg p-0 overflow-hidden fade-in-up max-h-[80vh] flex flex-col">
                            <div className="p-4 border-b border-lumina-border bg-lumina-surface flex justify-between items-center sticky top-0 z-10">
                                <div>
                                    <h3 className="font-bold text-white text-lg">{selectedProdForVariant.name}</h3>
